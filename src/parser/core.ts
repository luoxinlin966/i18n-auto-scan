...
import { cutBraceReg } from '../utils/constants'
  ...
  traverse(ast, {
    ...
    JSXText: function (path) {
      ...
      if (zhExt.test(path.toString())) {
        ...
        let id = generateId(value, options.templateInfo)
            ...
            t.callExpression(t.identifier(importLocal), [
              ...
              t.objectExpression(
                node.expressions.map((item, index) =>
                  t.objectProperty(t.stringLiteral(`${index + 1}`), item as any)
                )
              )
            ])
          )
...
function noLocale(value: string, id: string, relativePath: string, loc: any) {
  ...
  if (!baseLocale[id]) {
    ...
    let res = new Error(`[i18n-auto-scan]\n${relativePath}\n  ${content}`)
    // @ts-ignore
