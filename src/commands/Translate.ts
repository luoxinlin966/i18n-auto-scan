...
import {
  ...
  zhExt,
  generateId,
  generateTemplateLiteralParamsKey
} from '../utils'
import axios from 'axios'
import crypto from 'crypto'
import { isTSLiteralType } from '@babel/types'
import _traverse from '@babel/traverse'
import { unbracketReg, unSymbolReg, cutBraceReg } from '../utils/constants'
...
export class Translate {
    ...
    traverse(ast, {
      ...
      JSXText(path) {
        ...
        if (zhExt.test(path.toString())) {
          ...
          const id = generateId(value, _this.config.templateInfo)
        ...
        if (zhExt.test(path.toString())) {
          ...
          if (isCh) {
            ...
            let value = path
              .toString()
              .replace(/^`|`$/g, '')
              .replace(cutBraceReg, () => generateTemplateLiteralParamsKey(++i))
            const id = generateId(value, _this.config.templateInfo)
            _this.validateKey(value, id, file)
          }
        }
  ...
  formatChinese(word) {
    return word.chinese.replace(unbracketReg, (...args: any[]) => args[2])
  }
